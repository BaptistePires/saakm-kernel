...
static void nohz_csd_func(void *info)
{
	...
	if (rq->idle_balance && !need_resched()) {
		...
		raise_softirq_irqoff(SCHED_SOFTIRQ_SAAKM);
	}
}
...
int select_task_rq(struct task_struct *p, int cpu, int *wake_flags)
{
	...
	if ((p->nr_cpus_allowed > 1 && !is_migration_disabled(p)) || p->sched_class == &saakm_sched_class) {
...
#endif
	} else if (task_has_saakm_policy(p)) {
...
void sched_tick(void)
{
	...
	if (!scx_switched_all()) {
		...
		sched_balance_trigger_saakm(rq);
	}
#endif
}
...
#endif
	if (policy == SCHED_SAAKM)
		return &saakm_sched_class;
...
void __init sched_init(void)
{
	...
	BUG_ON(!sched_class_above(&fair_sched_class, &saakm_sched_class));
	BUG_ON(!sched_class_above(&saakm_sched_class, &idle_sched_class));
#ifdef CONFIG_SCHED_CLASS_EXT
