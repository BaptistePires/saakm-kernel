		...
		const struct sched_attr *attr)
{
	...
	} else if (saakm_policy(policy)) {
		__setparam_saakm(p, attr);
	}
...
recheck:
	...
	/*
	 * If switching to SCHED_SAAKM, check that the saakm policy exists
	 * and check cgroups to see if it's ok
	 */
	ipa_policy = attr->sched_ipa_policy;
	if (saakm_policy(policy)) {
		struct saakm_policy *cur_policy = NULL;
		int found = 0;
#ifdef CONFIG_CGROUP_SAAKM
		struct cgroup_subsys_state *css;
		struct saakm_group *ipa_grp;
		struct saakm_policy *cgrp_policy;
#endif	/* CONFIG_CGROUP_SAAKM */

		if (attr->sched_ipa_policy == -1 && saakm_task_policy(p))
			ipa_policy = saakm_task_policy(p)->id;

#ifdef CONFIG_CGROUP_SAAKM
		/*
		 * If process is in an saakm cgroup, check that it is not
		 * moving to another saakm policy
		 */
		css = p->cgroups->subsys[saakm_cgrp_id];
...
change:
	...
	running = task_current(rq, p);
	/*
	 * We also call dequeue_task() on tasks that use the saakm sched class
	 * if they are switching class to force the call to the terminate()
	 * handler, and decrease the refcnt of the saakm policy module. It is
	 * kind of hacky, but it works.
	 */
	if (queued || (saakm_policy(p->policy) &&
...
static int sched_copy_attr(struct sched_attr __user *uattr, struct sched_attr *attr)
{
	...
	/* Copy attributes of the saakm policy of necessary */
	if (saakm_policy(attr->sched_policy) &&
