...
int saakm_cfs_order_process(struct task_struct *a,
			       ...
			       struct process_event *e)
{
	...
	ipa_change_queue(tgt, NULL, SAAKM_TERMINATED, c->id);
	/* Memory barrier for proofs */
	smp_wmb();
	c->cload -= tgt->load;
	/* Memory barrier for proofs */
			      ...
			      struct process_event *e)
{
	...
	ipa_change_queue(tgt, NULL, SAAKM_BLOCKED, c->id);
	/* Memory barrier for proofs */
	smp_wmb();
	c->cload -= old_load;
	/* Memory barrier for proofs */
				       ...
				       struct process_event *e)
{
	...
	idlest = &saakm_core(task_cpu(task_15));
				  ...
				  struct core_event *e)
{
	...
	while (sd) {
		if (ktime_before(sd->next_balance, now)) {
			for (i = 0; i < sd->___sched_group_idx; i++) {
				sg = sd->groups + i;
				thief = &saakm_core(cpumask_first(&sg->cores));
...
int saakm_cfs_free_metadata(struct saakm_policy *policy)
{
	kfree(policy->data);
	return 0;
}

int saakm_cfs_can_be_default(struct saakm_policy *policy)
{
	return 1;
}
...
static int __init my_init_module(void)
{
	...
	/* Allocate & setup the saakm_module */
	...
	if (!procdir)
		pr_err("%s: /proc/saakm/%s creation failed\n", name, name);
	for_each_possible_cpu(cpu) {
		scnprintf(procbuf, 10, "%d", cpu);
		if (!proc_create(procbuf, 0444, procdir, &proc_fops))
			pr_err("%s: /proc/saakm/%s/%s creation failed\n",
			       name, name, procbuf);
	}
	if (!proc_create("topology", 0444, procdir, &proc_topo_fops))
		pr_err("%s: /proc/saakm/%s/topology creation failed\n",
		       name, name);
