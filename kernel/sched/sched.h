...
static inline int saakm_policy(int policy)
{
       return policy == SCHED_SAAKM;
}
...
static inline int task_has_saakm_policy(struct task_struct *p)
{
	return saakm_policy(p->policy);
}
...
extern bool __checkparam_saakm(const struct sched_attr *attr,
				 struct saakm_policy *policy);
extern void __setparam_saakm(struct task_struct *p,
			       const struct sched_attr *attr);
extern void __getparam_saakm(struct task_struct *p,
			       struct sched_attr *attr);
extern bool saakm_attr_changed(struct task_struct *p,
				 const struct sched_attr *attr);
...
#define UCLAMP_FLAG_IDLE 0x01
	...
	unsigned long           next_balance_saakm;
 ...
 *
 * The next flags are only used by the saakm sched class for now:
...
#define SWITCHING_CLASS         0x800
#define ATTR_CHANGE             0x1000
#define OUSTED                  0x2000
...
extern const struct sched_class saakm_sched_class;
extern const struct sched_class idle_sched_class;

/* SaaKM key*/
DECLARE_STATIC_KEY_FALSE(__saakm_policy_loaded);	/* Is there an saakm policy loaded */

#define saakm_enabled()	static_branch_unlikely(&__saakm_policy_loaded)
...
extern struct task_struct *_pick_next_task_saakm(struct rq *rq, struct task_struct *prev, struct rq_flags *rf);
...
extern void sched_balance_trigger_saakm(struct rq *rq);
...
extern void init_sched_saakm_class(void);
